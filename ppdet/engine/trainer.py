...
from ppdet.utils.visualizer import visualize_results,mask_image, save_result
            ...
            for outs in results:
                ...
                for i, im_id in enumerate(outs['im_id']):
                    ...
                    pose3d_res = batch_res['pose3d'][start:end] \
                            if 'pose3d' in batch_res else None
                    if mask:
                        image = mask_image(image_path, int(im_id), bbox_res, draw_threshold)
                        save_name = self._get_save_image_name(output_dir, image_path)
                        logger.info("Mask image results save in {}".format(save_name))
                        cv2.imencode('.png', image)[1].tofile(save_name)
                    else:
                        image = Image.open(image_path).convert('RGB') 
                        image = ImageOps.exif_transpose(image)  #相机翻转
                        self.status['original_image'] = np.array(image.copy())
                        image = visualize_results(   
                            image, bbox_res, mask_res, segm_res, keypoint_res,
                            pose3d_res, int(im_id), catid2name, draw_threshold)
